...
def preprocess_image(models, input_im, preprocess):
    ...
    if preprocess:
        ...
        print(np.shape(alpha), np.shape(input_im), np.shape(white_im))
...
def main_run(raw_im,
             ...
             preprocess=True,
             scale=6.0, n_samples=4, ddim_steps=50, ddim_eta=1.0,
...
def predict(device_idx: int =_GPU_INDEX,
    ...
    pred_image = pred_image.resize((width, height))
